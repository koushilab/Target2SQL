...
func main() {
	...
	creTab, err := db.Query("CREATE TABLE IF NOT EXISTS options(module VARCHAR(500),name VARCHAR(500), version_added VARCHAR(50), description VARCHAR(5000), required VARCHAR(100), defaults VARCHAR(1000), choices VARCHAR(100), elements VARCHAR(1000), aliases VARCHAR(100), suboptions VARCHAR(1000), type VARCHAR(1000) DEFAULT NULL)")
	PrintFatalError(err)
	defer creTab.Close()
	...
	for _, file := range files {
		...
		for i := 0; i < len(keys); i++ {
			...
			defer insForm.Close()
			/*
				fmt.Printf(" Keys: %v Description : %v\n", keys[i], Desc)
				fmt.Printf(" Keys: %v Default : %v\n", keys[i], Def)
				fmt.Printf(" Keys: %v Choices : %v\n", keys[i], Choice)
				fmt.Printf(" Keys: %v Suboptions : %v\n", keys[i], Suboptions)
			*/
			fmt.Println(count)
		}
	}
